<x-layouts.app title="TrustStack Dashboard">
    <div class="mb-8 flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div class="animate-fade-in-up">
            <p class="text-sm font-semibold uppercase tracking-[0.2em] text-opay-mint">ForgeSight dashboard</p>
        ...
        <a href="{{ route('verifications.create') }}" class="animate-fade-in-up delay-2 inline-flex items-center justify-center gap-2 rounded-lg bg-opay-mint px-5 py-3 text-sm font-bold text-white transition hover:bg-opay-mint/85 hover:shadow-lg hover:shadow-opay-mint/20 active:scale-[0.98]">
    ...
    <section class="mb-8 grid gap-4 md:grid-cols-4">
        ...
        <div class="glass hover-lift animate-fade-in-up delay-2 rounded-xl p-5">
            <div class="flex items-center justify-between">
                ...
                <div class="grid h-8 w-8 place-items-center rounded-lg bg-opay-teal/15">
                    <svg class="h-4 w-4 text-opay-teal" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                </div>
            </div>
            <div class="mt-3 text-3xl font-bold text-opay-teal">{{ $paidPayments }}</div>
        </div>
                ...
                <tbody class="divide-y divide-white/[0.06]">
                    @forelse ($verifications as $verification)
                        ...
                        <tr class="transition hover:bg-white/[0.03]">
                            ...
                            <td class="px-5 py-4">
                                <span class="rounded-md border px-2 py-1 text-xs {{ $verification->payment?->status === 'paid' ? 'border-opay-teal/30 bg-opay-teal/10 text-emerald-200' : 'border-white/10 bg-white/[0.03] text-slate-300' }}">
                            ...
                            <td class="px-5 py-4 text-right">
                                <a href="{{ route('verifications.show', $verification) }}" class="inline-flex items-center gap-1 text-sm font-medium text-opay-mint transition hover:text-opay-mint/80">
                                    Open
