<x-layouts.app title="New TrustStack Scan">
    <div class="mb-8 animate-fade-in-up">
        <p class="text-sm font-semibold uppercase tracking-[0.2em] text-opay-mint">New verification</p>
        ...
        <section class="animate-fade-in-up delay-1 glass rounded-xl p-6">
            <div class="grid gap-5 sm:grid-cols-2">
                <label class="block sm:col-span-2">
                    ...
                    <select name="institution_id" required class="mt-2 w-full rounded-lg border border-white/10 bg-slate-950/80 px-3 py-3 text-sm text-white outline-none transition focus:border-opay-mint focus:ring-1 focus:ring-opay-mint/30">
                ...
                <label class="block">
                    ...
                    <input name="candidate_name" value="{{ old('candidate_name') }}" required class="mt-2 w-full rounded-lg border border-white/10 bg-slate-950/80 px-3 py-3 text-sm text-white outline-none transition focus:border-opay-mint focus:ring-1 focus:ring-opay-mint/30">
                ...
                <label class="block">
                    ...
                    <input name="candidate_email" type="email" value="{{ old('candidate_email') }}" required class="mt-2 w-full rounded-lg border border-white/10 bg-slate-950/80 px-3 py-3 text-sm text-white outline-none transition focus:border-opay-mint focus:ring-1 focus:ring-opay-mint/30">
                ...
                <div class="sm:col-span-2">
                    ...
                    <div id="drop-zone" class="group mt-2 cursor-pointer rounded-xl border-2 border-dashed border-white/15 bg-slate-950/60 px-6 py-10 text-center transition hover:border-opay-mint/40 hover:bg-opay-mint/[0.02]">
                        <input name="document" id="file-input" type="file" accept="image/png,image/jpeg,image/webp" required class="hidden">

                        <div id="upload-prompt">
                            <svg class="mx-auto h-10 w-10 text-slate-500 transition group-hover:text-opay-mint/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5"/></svg>
                            <p class="mt-3 text-sm font-medium text-slate-300">Drop your certificate here or <span class="text-opay-mint underline underline-offset-2">browse files</span></p>
                        ...
                        <div id="upload-preview" class="hidden">
                            ...
                            <p class="mt-1 text-xs text-opay-mint">Click to change file</p>
                        </div>
                    </div>
            ...
            <div class="mt-6 flex items-center justify-end gap-3">
                ...
                <button class="rounded-lg bg-opay-mint px-5 py-3 text-sm font-bold text-white transition hover:bg-opay-mint/85 hover:shadow-lg hover:shadow-opay-mint/20 active:scale-[0.98]">Continue to payment</button>
            </div>
        </section>

        <aside class="animate-fade-in-up delay-2 glass rounded-xl p-6">
            <h2 class="text-base font-bold text-white">Fee allocation</h2>
            <p class="mt-1 text-xs text-slate-500">Powered by OPay Payment API</p>
            <div class="mt-5 space-y-4">
                ...
                <div class="flex items-center justify-between border-b border-white/10 pb-3">
                    <span class="text-sm text-slate-400">TrustStack service fee</span>
            ...
            <div class="mt-5 rounded-lg border border-cyan-300/20 bg-cyan-300/5 p-3">
                <div class="flex items-start gap-2">
                    ...
                    <p class="text-xs leading-relaxed text-cyan-100/70">The ₦1,000 royalty is <strong class="text-white">automatically transferred</strong> to the issuing university's bank account after payment clears, via OPay Transfer API.</p>
                </div>
            </div>
            ...
            <div class="mt-6 border-t border-white/[0.06] pt-4">
                ...
                <ol class="mt-3 space-y-2 text-xs text-slate-400">
                    <li class="flex items-start gap-2">
                        <span class="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-opay-mint/10 text-[10px] font-bold text-opay-mint">1</span>
                        <span>Upload certificate and pay ₦5,000 via OPay</span>
                    </li>
                    <li class="flex items-start gap-2">
                        <span class="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-opay-mint/10 text-[10px] font-bold text-opay-mint">2</span>
                        <span>AI engine runs ELA + OCR forensic analysis</span>
                    </li>
                    <li class="flex items-start gap-2">
                        <span class="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-opay-mint/10 text-[10px] font-bold text-opay-mint">3</span>
                        <span>₦1,000 royalty auto-routed to university bank</span>
                    </li>
                    <li class="flex items-start gap-2">
                        <span class="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-opay-mint/10 text-[10px] font-bold text-opay-mint">4</span>
    ...
    <script>
        (() => {
            ...
            zone.addEventListener('dragover', e => {
                e.preventDefault();
                zone.classList.add('border-opay-mint/50', 'bg-opay-mint/5');
            });

            zone.addEventListener('dragleave', () => {
                zone.classList.remove('border-opay-mint/50', 'bg-opay-mint/5');
            });
