<x-layouts.app title="New TrueSeal Scan">
    <div class="mb-8 animate-fade-in-up">
        <p class="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-300">New verification</p>
        ...
        <section class="animate-fade-in-up delay-1 glass rounded-xl p-6">
            <div class="grid gap-5 sm:grid-cols-2">
                <label class="block sm:col-span-2">
                    ...
                    <select name="institution_id" required class="mt-2 w-full rounded-lg border border-white/10 bg-slate-950/80 px-3 py-3 text-sm text-white outline-none transition focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400/30">
                ...
                <label class="block">
                    ...
                    <input name="candidate_name" value="{{ old('candidate_name') }}" required class="mt-2 w-full rounded-lg border border-white/10 bg-slate-950/80 px-3 py-3 text-sm text-white outline-none transition focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400/30">
                ...
                <label class="block">
                    ...
                    <input name="candidate_email" type="email" value="{{ old('candidate_email') }}" required class="mt-2 w-full rounded-lg border border-white/10 bg-slate-950/80 px-3 py-3 text-sm text-white outline-none transition focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400/30">
                ...
                <div class="sm:col-span-2">
                    ...
                    <div id="drop-zone" class="group mt-2 cursor-pointer rounded-xl border-2 border-dashed border-white/15 bg-slate-950/60 px-6 py-10 text-center transition hover:border-emerald-400/40 hover:bg-emerald-400/[0.02]">
                        <input name="document" id="file-input" type="file" accept="image/png,image/jpeg,image/webp" required class="hidden">

                        <div id="upload-prompt">
                            <svg class="mx-auto h-10 w-10 text-slate-500 transition group-hover:text-emerald-400/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5"/></svg>
                            <p class="mt-3 text-sm font-medium text-slate-300">Drop your certificate here or <span class="text-emerald-300 underline underline-offset-2">browse files</span></p>
                        ...
                        <div id="upload-preview" class="hidden">
                            ...
                            <p class="mt-1 text-xs text-emerald-300">Click to change file</p>
                        </div>
                    </div>
            ...
            <div class="mt-6 flex items-center justify-end gap-3">
                ...
                <button class="rounded-lg bg-emerald-400 px-5 py-3 text-sm font-bold text-slate-950 transition hover:bg-emerald-300 hover:shadow-lg hover:shadow-emerald-400/20 active:scale-[0.98]">Continue to payment</button>
            </div>
        </section>
            ...
            <div class="mt-6 border-t border-white/[0.06] pt-4">
                ...
                <ol class="mt-3 space-y-2 text-xs text-slate-400">
                    <li class="flex items-start gap-2">
                        <span class="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-emerald-400/10 text-[10px] font-bold text-emerald-300">1</span>
                        <span>Upload certificate and pay ₦5,000 via Squad</span>
                    </li>
                    <li class="flex items-start gap-2">
                        <span class="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-emerald-400/10 text-[10px] font-bold text-emerald-300">2</span>
                        <span>AI engine runs ELA + OCR forensic analysis</span>
                    </li>
                    <li class="flex items-start gap-2">
                        <span class="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-emerald-400/10 text-[10px] font-bold text-emerald-300">3</span>
                        <span>₦1,000 royalty auto-routed to university bank</span>
                    </li>
                    <li class="flex items-start gap-2">
                        <span class="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-emerald-400/10 text-[10px] font-bold text-emerald-300">4</span>
    ...
    <script>
        (() => {
            ...
            zone.addEventListener('dragover', e => {
                e.preventDefault();
                zone.classList.add('border-emerald-400/50', 'bg-emerald-400/5');
            });

            zone.addEventListener('dragleave', () => {
                zone.classList.remove('border-emerald-400/50', 'bg-emerald-400/5');
            });
