    ...
    <div class="gradient-bg grid-pattern -mx-4 -mt-8 px-4 py-10 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
        <div class="mx-auto grid max-w-5xl items-center gap-10 py-6 lg:grid-cols-[1.1fr_0.9fr]">
            <section class="animate-fade-in-up">
                <p class="mb-3 text-sm font-semibold uppercase tracking-[0.2em] text-emerald-300">Institutional trust layer</p>
                <h1 class="max-w-2xl text-4xl font-bold leading-tight text-white sm:text-5xl">Forensic certificate verification with <span class="text-emerald-300">royalty routing.</span></h1>
                ...
                <div class="mt-8 flex items-center gap-4">
                    <div class="flex -space-x-2">
                        <div class="h-8 w-8 rounded-full border-2 border-[#070b12] bg-emerald-500/30 grid place-items-center text-xs font-bold text-emerald-300">U</div>
            ...
            <section class="glass-strong animate-fade-in-up delay-3 rounded-xl p-6 shadow-2xl shadow-black/40">
                ...
                <p class="mt-1 text-sm text-slate-400">Demo: <span class="font-mono text-emerald-300/80">hr@trueseal.test</span> / <span class="font-mono text-emerald-300/80">password</span></p>

                <form method="POST" action="{{ route('login') }}" class="mt-6 space-y-4">
                    @csrf
                    <label class="block">
                        <span class="text-sm font-medium text-slate-300">Email</span>
                        <input name="email" type="email" value="{{ old('email', 'hr@trueseal.test') }}" required autofocus class="mt-2 w-full rounded-lg border border-white/10 bg-slate-950/80 px-4 py-3 text-sm text-white outline-none transition focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400/30">
                        @error('email')
                    ...
                    <label class="block">
                        ...
                        <input name="password" type="password" required class="mt-2 w-full rounded-lg border border-white/10 bg-slate-950/80 px-4 py-3 text-sm text-white outline-none transition focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400/30">
                        @error('password')
                            <span class="mt-2 block text-sm text-red-300">{{ $message }}</span>
                        @enderror
                    </label>
                    ...
                    <label class="flex items-center gap-2 text-sm text-slate-400">
                        <input name="remember" type="checkbox" class="rounded border-white/20 bg-slate-950 text-emerald-400 focus:ring-emerald-400/30">
                        Keep this session active
                    </label>

                    <button class="w-full rounded-lg bg-emerald-400 px-4 py-3 text-sm font-bold text-slate-950 transition hover:bg-emerald-300 hover:shadow-lg hover:shadow-emerald-400/20 active:scale-[0.98]">Enter dashboard</button>
                </form>
                ...
                <div class="mt-5 flex items-center gap-2 rounded-lg border border-white/[0.06] bg-white/[0.02] px-3 py-2.5 text-xs text-slate-500">
                    <svg class="h-4 w-4 shrink-0 text-emerald-400/60" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
